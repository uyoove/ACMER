...
struct Matrix
{
    ...
    Matrix(int a) : r(a), c(a) {memset(ele, 0, sizeof(ele));for(int i = 0;i < a;i++)ele[i][i] = 1;} //单位矩阵
...
Matrix<m, n> qpowmod(Matrix<m, n>& mat, long long k, int mod) //快速幂取模
{
    ...
    for (int p = k; p; mat = slowmulmod(mat, mat, mod), p >>= 1)
        if (p & 1)
            ans = slowmulmod(ans, mat, mod);
    return ans;
}
template <int m, int n>
...
int js(int x1){
	...
	//cout << mat << endl;
	//cout << mat2 << endl;
	auto p = qpowmod(mat2,x1,m);
	auto res = mulmod(p,mat,m);
	//cout << res <<endl;
	return res(0,0);
}
void solve(){
